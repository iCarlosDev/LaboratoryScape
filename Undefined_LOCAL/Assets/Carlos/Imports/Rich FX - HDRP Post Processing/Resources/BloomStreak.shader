Shader "Hidden/InanEvin/RichFX/BloomStreak"
{
	HLSLINCLUDE
...
#include "Packages/com.unity.render-pipelines.high-definition/Runtime/ShaderLibrary/ShaderVariables.hlsl"
		...
		float4 FragmentComposition(Varyings input) : SV_Target
	{
		...
		float3 c4 = (c0 / 4 + c1 / 2 + c2 / 4);

		float3 rgb = LinearToSRGB((c4 * _Color).rgb);
		...
		// sRGB -> Linear
		c4.rgb = SRGBToLinear(rgb);

		float3 cf = c4  * _Intensity * 5;
